...
/*   Updated: 2024/06/12 14:22:19 by tfreydie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	handle_too_many_args(bool is_solo_b, t_gc **gc, int backup_fds[2]);
...
int	ft_exit(char **args, t_gc *gc, int backup_fds[2], bool is_solo_b)
{
	...
	if (too_many_arguments(args) == true)
		return (handle_too_many_args(is_solo_b, &gc, backup_fds));
	close_backup_fds(backup_fds);
	if (is_solo_b && ft_printf("exit\n") == -1)
		perror_exit(gc, errno, WRITE_ERR_MSG);
